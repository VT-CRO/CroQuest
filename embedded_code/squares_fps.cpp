...
void updateFPSDisplay() {
  tft.fillRect(0, tft.height() - 20, 160, 20, ILI9341_BLACK);
  tft.setCursor(5, tft.height() - 18);
  tft.setTextColor(ILI9341_YELLOW);
  tft.print("Phase ");
  tft.print(currentPhase + 1);
  tft.print(": ");
  tft.print(fps);
  tft.print(" FPS");
}
...
void loop() {
  ...
  if (now - phaseStartTime >= 5000) { // 5 seconds per phase
    phaseFPS[currentPhase] = phaseFrameCount / 5; // average FPS
    currentPhase++;
