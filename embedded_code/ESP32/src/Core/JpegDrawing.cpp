...
void JpegDrawing::drawJpegTile(const char *filename, int srcX, int srcY, int w,
  ...
  // Create temp sprite for the tile
  TFT_eSprite tileSprite = TFT_eSprite(&tft);
  tileSprite.setColorDepth(8);
  tileSprite.createSprite(w, h);

  for (int m = 0; JpegDec.read(); m++) {
    ...
    if (mcuX + mcuW <= srcX || mcuX >= srcX + w || mcuY + mcuH <= srcY ||
        mcuY >= srcY + h)
      continue; // Skip MCUs outside the tile

    for (int y = 0; y < mcuH; y++) {
      for (int x = 0; x < mcuW; x++) {
        int px = mcuX + x;
        int py = mcuY + y;
        if (px >= srcX && px < srcX + w && py >= srcY && py < srcY + h) {
          int localX = px - srcX;
          int localY = py - srcY;
          tileSprite.drawPixel(localX, localY, JpegDec.pImage[y * mcuW + x]);
        }
      }
    }
  }

  // Paste the tile into the main sprite
