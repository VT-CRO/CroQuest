// src/SettingsMenu/NameMenu/Name.cpp
...
static bool isProhibitedName(const std::string &input);
...
static void handleDrawing(char name[], NameSelection selection) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if (i == prevSelectedLetterIndex &&
        prevSelectedLetterIndex != selectedLetterIndex) {
      // Remove highlight
      tft.drawRect(x - 2, y - 2, boxSize + 4, boxSize + 4, SETTINGS_BG_COLOR);

      // remove arrows
      tft.fillRect(x + boxSize / 2 - base / 2, y - height - 6, base + 4,
                   height + 4, SETTINGS_BG_COLOR); // up
      tft.fillRect(x + boxSize / 2 - base / 2, y + boxSize + 4, base + 4,
                   height + 4, SETTINGS_BG_COLOR); // down
...
std::vector<String> loadProhibitedNames() {
  std::vector<String> names;
  File file = SD.open("/.name/prohibited_names.txt");
  if (!file) {
    Serial.println("Prohibited names file not found!");
    return names;
  }

  while (file.available()) {
    String name = file.readStringUntil('\n');
    name.trim(); // Remove newline and whitespace
    if (name.length() == 5) {
