...
*/
int min(int value1, int value2) { return value1 >= value2 ? value2 : value1; }
...
void initialize_game(Ball *ball, Paddle *paddles, int *level) {
  ...
  *level = 1;
  ...
  paddles[0] = (Paddle){0, (SCREEN_HEIGHT / 2) - (PADDLE_HEIGHT / 2),
                        PADDLE_WIDTH, PADDLE_HEIGHT};
  paddles[1] = (Paddle){SCREEN_WIDTH - PADDLE_WIDTH,
                        (SCREEN_HEIGHT / 2) - (PADDLE_HEIGHT / 2), PADDLE_WIDTH,
                        PADDLE_HEIGHT};
}
...
*/
static enum paddle_walls closest_wall(Paddle paddle, int inside_dist,
                                      int top_dist, int bottom_dist) {
...
static void check_paddle_collision(Ball *ball, Paddle *paddles, int *level) {
  for (int i = 0; i < 2; i++) {
    ...
    if (ball->x + ball->w < paddle->x || ball->x > paddle->x + PADDLE_WIDTH ||
        ball->y + ball->h < paddle->y || ball->y > paddle->y + PADDLE_HEIGHT) {
      continue; // No collision with this paddle
    }

    // Calculate distances to each wall
    int inside_dist = (i == 0) ? abs(ball->x - (paddle->x + PADDLE_WIDTH))
    ...
    enum paddle_walls minimum =
        closest_wall(*paddle, inside_dist, top_dist, bottom_dist);
    ...
    float normalized_offset =
        (float)(ball->y - paddle_center) / (PADDLE_HEIGHT / 2);

    // Apply a sinusoidal curve to move max velocity points inward
    // This creates max velocity at around 2/3 from center instead of at the
    // edge
    ...
    if (minimum == TOP) {
      ...
      if (i == 0) { // Left paddle
        ...
        ball->dx =
            fabs(ball->dx) * (1.0 + (*level) * 0.05); // Gradual speed increase
      } else {                                        // Right paddle
...
*/
static void ball_collision(Ball *ball, Paddle *paddles, int *level, int *score0,
                           int *score1) {
...
void ai_paddle(Paddle *paddle, Ball *ball, int level_index) {
  ...
  if (ball->x > SCREEN_WIDTH / 2) {
    ...
    double curiosity_factor =
        1.0 - ((ball->x - SCREEN_WIDTH / 2) / (SCREEN_WIDTH / 2));
    if (curiosity_factor < 0)
      curiosity_factor = 0;
    if (curiosity_factor > 1)
      curiosity_factor = 1;
  ...
  y_intersect = y_intersect >= SCREEN_HEIGHT ? SCREEN_HEIGHT - PADDLE_HEIGHT
                                             : y_intersect;
  ...
  double mistake_range =
      (ball_speed / 5.0) * (1.0 / (intersection_time + 0.1)); // avoid div by 0
  ...
  // Clamp mistake range
  if (mistake_range > PADDLE_HEIGHT / 2)
    mistake_range = PADDLE_HEIGHT / 2;
  if (mistake_range < 2)
    mistake_range = 2;

  // Add error to the target
  double error =
      ((random() % 100) / 100.0 - 0.5) * 2 * mistake_range; // [-range, +range]
  double target_y = y_intersect + error;

  // Clamp within screen bounds
  target_y =
      target_y >= SCREEN_HEIGHT ? SCREEN_HEIGHT - PADDLE_HEIGHT : target_y;
